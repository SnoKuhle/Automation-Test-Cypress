describe('Wonder Lab Test Spec', () => {
  ...
  it.only('Check Boxes', () => {
    ...
    // Validating that only radio button 2 is selected
    cy.get('#checkbox-example > fieldset').find('label').each(($label, index) => {
      cy.wrap($label).find('input').check().should('be.checked')
    })
    ...
    cy.get('#checkbox-example > fieldset').find('label').each(($label, index) => {
      if(index == 0)
          cy.wrap($label).find('input').uncheck().should('not.be.checked')
